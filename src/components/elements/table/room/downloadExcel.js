...
export default function DownloadExcelRuangan() {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/ruangan/report/excel`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = "Report Ruangan"; // Nama file yang akan digunakan saat menyimpan.
        a.click();
      }
    } catch (error) {
